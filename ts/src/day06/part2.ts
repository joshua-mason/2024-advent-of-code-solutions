...
async function part2() {
  ...
  const tilesSteppedOn: [number, number][] = [];
  labMap.forEach(({ tile, stepCount }, key) => {
  ...
  let detectedLoops = 0;
  for (const tile of tilesSteppedOn) {
    const { labMap, startingGuardPosition } = buildLabMap(testDataToUse);
    labMap.set(tile.join(','), { stepCount: 0, tile: Tile.Obstruction });
    const completed = simulateGuardMovement({
      labMap,
      startingGuardPosition,
      width,
      height,
...
function simulateGuardMovement({
  ...
  while (
    ...
    const tileSteppedOn = labMap.get(moveDetails.guardPosition.join(','));
    if (tileSteppedOn && tileSteppedOn.stepCount > 20) {
      return false;
    }
